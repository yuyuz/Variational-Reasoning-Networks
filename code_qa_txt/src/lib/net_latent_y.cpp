...
void NetLatentY::BuildBatchGraph(std::vector< Sample* >& mini_batch, Phase phase)
{    
...
std::shared_ptr< DTensorVar<mode, Dtype> > NetLatentY::GetMatchScores(std::shared_ptr< DTensorVar<mode, Dtype> >& q_embed, 
...
std::shared_ptr< DTensorVar<mode, Dtype> > NetLatentY::GetCritic(std::shared_ptr< SpTensorVar<mode, Dtype> > q_bow)                                                                 
{
...
void NetLatentY::BuildNet()
{
    ...
    q_embed_query = af< MatMul >(fg, {q_bow, w_path_query});
    auto entity_bow_embed = af< MatMul >(fg, {entity_bow, w_entity_match}); 
    q_y_bow_match = af< MatMul >(fg, {q_embed_entity, entity_bow_embed}, Trans::N, Trans::T);
    ...
    // should do sampling
    pos_probs = af< Softmax >(fg, {q_y_given_qa_scores});

    sampled_y_idx = af< MultinomialSample >(fg, {pos_probs}, false);
    
    auto sampled_y_nodes = af< NodeSelect >(fg, {sampled_y_idx});
    ...
    auto one_hot_sampled_y = af< OneHot<mode, Dtype> >(fg, {sampled_y_idx}, kb.node_dict.size());
    ...
    std::vector< std::shared_ptr< Variable > > tmp = {sampled_y_idx, learning_signal};
    auto onehot_joint = af< OneHot<mode, Dtype> >(fg, tmp, kb.node_dict.size());
    auto pos_neg_loss = af< CrossEntropy >(fg, {pos_probs, onehot_joint}, false);
    //auto pos_neg_loss = af< CrossEntropy >(fg, {q_y_given_qa_scores, onehot_joint}, true);
    ...
    //================ inference ===========================
    auto argmax_y = af< ArgMax >(fg, {q_y_bow_match});
    auto infer_y_nodes = af< NodeSelect >(fg, {argmax_y});
    pred = GetMatchScores(q_embed_query, samples, rel_embed, w_y2a_recur, infer_y_nodes);

    hitk = af< HitAtK >(fg, {pred, ans_output});
